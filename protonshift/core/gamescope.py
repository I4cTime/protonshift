...
Lifted from the original ProtonShift Python core. The only behavioural change
from the source is the FSR fix (see ``build_gamescope_argv``): the upstream
version emitted ``--fsr-sharpness`` without the flag that actually turns FSR on,
so the toggle was a no-op. Fixed here.
"""
...
from .tool_check import find_scopebuddy, is_tool_available
...
def build_gamescope_argv(opts: GamescopeOptions) -> list[str]:
    ...
    if opts.fsr:
        # FIX (was ProtonShift issue #24): the flag that actually enables FSR
        # upscaling must be emitted, not just the sharpness value. `-F fsr` is
        # the current gamescope spelling (`--filter fsr`).
        parts.extend(["-F", "fsr"])
